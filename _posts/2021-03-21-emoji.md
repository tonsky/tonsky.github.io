...
category: blog
summary: "Detailed look into all the machinery involved into rendering Emoji"
draft: true
---
...
For the past few weeks, I’ve been implementing emoji support for [Skija](https://github.com/jetbrains/skija). I thought it might be fun sharing a few nitty-gritty details of how this “biggest innovation in human communication since the invention of letter 🅰️” works under the hood.

Warning: some emoji might not display as expected on your device. In that case, use this text version: ¯\\\_(ツ)\_/¯

# Intro to Unicode

As you might know, all text inside computers is encoded with numbers. One letter—one number. The most popular encoding we use is called Unicode, with two most popular variations called UTF-8 and UTF-16.

Unicode allocates 2²¹ (\~2 mil) characters, or codepoints. Sorry, programmers, but it’s not a multiply of 8 🤷. Out of those 2 millions, actually defined are \~150k characters.
...
That’s why emoji behave like any other letter: they can be typed in a textfield, copied, pasted, rendered in a plain text document, embedded in a tweet etc. When you type “A”, computer sees U+0041. When you type “🌵”, computer sees U+1F335. Not much difference.
...
Every OS comes with a single pre-installed font for emoji. On masOS/iOS, that’s Apple Color Emoji. Windows has Segoe UI Emoji, Android has Noto Color Emoji.
...
Well, with the same machinery that makes cyrillic text look ugly in Clubhose or on Medium: font fallbacks.
...
Simple, elegant, and no need to allocate new codepoints while the old ones are already there. After all, things like ☠︎ and ☠️ have the same _meaning_, only presentation is different.

## Grapheme clusters

Here we encounter another problem — our emoji are now not a single codepoint, but two. Which means we need a way to define character boundaries.

Meet Grapheme Clusters. Grapheme cluster is a sequence of codepoints that is considered a single human-percieved glyph.

Grapheme Clusters were not invented just for emoji, they are applicable to normal alphabets too. “Ü” is a single grapheme cluster, even though it’s composed of two codepoints: U+0055 UPPER-CASE U followed by U+0308 COMBINING DIAERESIS.

Grapheme clusters create many complications for programmers. You can’t just do `substring(0, 10)` to take first 10 characters—you might split an emoji in half (or an acute, so don’t do it anyway)!
...
A tip for programmers: if you are working with text, get a library that is grapheme clusters-aware. For C, C++ and JVM that would be [ICU](http://site.icu-project.org/), Swift does the right thing out-of-the-box, for other, see for yourself.
...
These are not supposed to be used on their own, but appended to the existing emoji. Together they form a ligature: 👋 (U+1F44B WAVING HAND SIGN) directly follwed by 🏽 (U+1F3FD MEDIUM SKIN TONE MODIFIER) becomes 👋🏽.
...
U+200D is called ZERO-WIDTH JOINER, or ZWJ for short. It works similarly to what we saw with skin tone, but this time you can join two self-sufficient emoji into one. Not all combinations work, but many do, sometimes in a surprising ways!
...
Unfortunately, some emoji are NOT implemented as combinations with ZWJ. Missing opportunities:
...
Country flags are part of Unicode standard, but for some reason are not implemented on Windows. If you are reading this in a browser from Windows—I am sorry!

Flags don’t have dedicated codepoints. Instead, they are composed as two-letter ligatures.
...
They don’t use real letters, though, instead “regional indicator symbol letter” alphabet is used (U+1F1E6..1F1FF). These letters are not used for anything but composing flags.
...
Tag sequence is a sequence of normal emoji, followed by another flavour of latin letters (U+E0020..E007E), terminated with U+E007F CANCEL TAG.
...
At the moment of writing neither latest macOS (11.2.3) nor iOS (14.4.1) support emoji from Unicode 13 like
...
If you are a programmer, remember to always use ICU library to:
